...
public class DateTimeKit {
    ...
    /***年月时间格式***/
    public final static String MOUTH_PATTERN = "yyyyMM";
    /**
     * 标准日期格式月份
     */
    public final static String NORM_MOUTH_PATTERN = "yyyy-MM";
    /**
     * 标准时间格式
     */
     ...
     *
     */
    public final static String NORM_DATETIME_MSS_PATTERN = "yyyyMMddHHmmssSSS";

    /**
     *
     */
    public final static String NORM_DATETIME_DAY_PATTERN = "yyyyMMddHH";
    /**
     * HTTP头中日期时间格式
     */
    ...
    public static DateTime parse(String dateStr, SimpleDateFormat simpleDateFormat) {
        try {
            ...
            throw new BusinessException("解析日期异常", ResultEnum.TOOL_EXP.getCode());
        }
    }
     ...
     * @return
     */
    public static long currentExpriseTime() {
        DateTime to = offsiteDay(DateTimeKit.parse(DateTimeKit.format(new Date(), DateTimeKit.NORM_DATE_PATTERN)), 1);
        long time = to.getTime() - System.currentTimeMillis();
        if (time <= 1000) {
            return 1;
        } else {
    ...
    /**
     * 计算2个时间区间的月份数组 string[]{'2018-06','2018-05'}
     *
     * @param startDate yyyy-MM-dd 或 yyyy-MM
     * @param endDate   yyyy-MM-dd 或 yyyy-MM
     * @return yyyyMM的格式
     */
    public static List<String> countMiddleOfTwoDays(String startDate, String endDate) {
    ...
    /**
     * 指定时间与当前时间的时间搓
     * @param days > 0 当前日期向后偏移，< 0 当前时间往前偏移，= 0为当前时间
     * @return 返回秒值:
     */
    public static long currentTimeOfDate(int days){
        long times = System.currentTimeMillis();
        long endTimes = DateTimeKit.parse(DateTimeKit.format(DateTimeKit.offsiteDay(new Date(),days),DateTimeKit.NORM_DATE_PATTERN)).getTime();
        return (endTimes - times) / 1000;
    }
     ...
     * @return
     */
    public static String longTime2String(long time,String format){
        return DateTimeKit.format(DateTimeKit.date(time),format);
    }
